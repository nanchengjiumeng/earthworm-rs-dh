...
use windows::Win32::UI::WindowsAndMessaging::{
  EnumChildWindows, EnumWindows, GetAncestor, GetShellWindow, GetWindowLongW, IsWindowVisible,
...
extern "system" fn enum_child_window(window: HWND, state: LPARAM) -> BOOL {
  unsafe {
    let state = Box::leak(Box::from_raw(state.0 as *mut WindowEnumerationState));
    if let Some(console_window) = &state.console_window {
      if window == *console_window {
...
pub fn find_sub_window(window: &WindowInfo, sub_window_title: String) -> Vec<WindowInfo> {
  unsafe {
    let state = Box::into_raw(Box::new(WindowEnumerationState {
      windows: Vec::new(),
      console_window: Some(window.handle),
    }));
    EnumChildWindows(
      window.handle,
    ...
    for window_info in state.windows.into_iter() {
      let title = window_info.title.to_lowercase();
      if title == sub_window_title.to_lowercase() {
